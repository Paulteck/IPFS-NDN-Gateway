...
type Session interface {
	exchange.Fetcher
	ID() uint64
	ReceiveFrom(peer.ID, []cid.Cid, []cid.Cid, []cid.Cid, map[cid.Cid]int)
	Shutdown()
}
...
// MARS todo wantedc einbauen
// todo neue struct?, oder par/count
// ReceiveFrom is called when a new message is received
func (sm *SessionManager) ReceiveFrom(ctx context.Context, p peer.ID, blks []cid.Cid, haves []cid.Cid, dontHaves []cid.Cid, blksc map[cid.Cid]int) {
	//fmt.Println("Debug: sm-receivefrom blk", blks)
	...
	for _, id := range sm.sessionInterestManager.InterestedSessions(blks, haves, dontHaves, blksc) {
		sm.sessLk.RLock()
		if sm.sessions == nil { // check if SessionManager was shutdown
			sm.sessLk.RUnlock()
			return
		...
		if ok {
			sess.ReceiveFrom(p, blks, haves, dontHaves, blksc)
		}
	}

	cancel := make([]cid.Cid,0,len(blks))
	for _, key:=range blks{
		if !sm.sessionInterestManager.IsCodedInterest(key){
			cancel = append(cancel, key)
		}
	}
	fmt.Println("Debug: sessman-receivefrom blk", blks, "have", haves)
	// Send CANCEL to all peers with want-have / want-block
	sm.peerManager.SendCancels(ctx, cancel)
}
...
func (sm *SessionManager) cancelWants(wants []cid.Cid) {
	//fmt.Println("Debug: sm-cancelwants ", wants)
