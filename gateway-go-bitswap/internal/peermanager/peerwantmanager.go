...
type peerWant struct {
	wantBlocks *cid.Set
	wantHaves  *cid.Set
	wantCodeds  *map[CodedWant]struct{}
	peerQueue  PeerQueue
}

type CodedWant struct{
	Cid cid.Cid
	Count int
	Coding string
}
...
func (pwm *peerWantManager) addPeer(peerQueue PeerQueue, p peer.ID) {
	...
	wantCodeds:=make(map[CodedWant]struct{})
...
func (pwm *peerWantManager) broadcastWantHaves(wantHaves []cid.Cid) {
	...
	for _, c := range wantHaves {
		if pwm.broadcastWants.Has(c) {
			fmt.Println("Debug: existing broadcast want ", c)
	...
	for _, pws := range pwm.peerWants {
		...
		for _, c := range unsent {
			...
			if !pws.wantBlocks.Has(c) && !pws.wantHaves.Has(c) {
				...
				fmt.Println("Debug: skipping peer")
			}
		}

		fmt.Println("Debug: pwm - sending bcst to", len(peerUnsent))
...
func (pwm *peerWantManager) sendWants(p peer.ID, wantBlocks []cid.Cid, wantHaves []cid.Cid, wantCodeds []CodedWant) {
	...
	// MARS todo Vorbetrachtung
	ckeys := make([]CodedWant, len(wantCodeds))
	i := 0
	for _, k := range wantCodeds {
		ckeys[i] = k
		i++
	}

	//fmt.Println("Debug: pwm-sendWants coded", i)

	// Send the want-blocks and want-haves to the peer
	pws.peerQueue.AddWants(fltWantBlks, fltWantHvs, ckeys)
}
...
func (pwm *peerWantManager) sendCancels(cancelKs []cid.Cid) {
	//fmt.Println("Debug: pwm-sendcancels", cancelKs)
	if len(cancelKs) == 0 {
		return
	}
	...
	for _, c := range broadcastCancels {
		fmt.Println("Debug: Removing broadcast want ", c)
...
func (pwm *peerWantManager) sendCancelC(p peer.ID, c cid.Cid, count int) {
	pws, ok := pwm.peerWants[p]
	if !ok {
		// In practice this should never happen
		log.Errorf("sendWants() called with peer %s but peer not found in peerWantManager", string(p))
	...
	pws.peerQueue.AddCancelC(c, count)
