...
type PubSub interface {
	...
	SubscribeC(ctx context.Context, key cid.Cid, coding string, count int) <-chan blocks.Block
	Shutdown()
}
...
func (ps *impl) Publish(block blocks.Block) {
	...
	if bl, ok:= block.(*blocks.CodedBlock); ok{
		ps.wrapped.Pub(bl, bl.Parent().KeyString())
	} else {
...
func (ps *impl) SubscribeC(ctx context.Context, key cid.Cid, coding string, count int) <-chan blocks.Block {
	c := 0

	blocksCh := make(chan blocks.Block, count)
	valuesCh := make(chan interface{}, count) // provide our own channel to control buffer, prevent blocking
	if count == 0 {
		close(blocksCh)
		return blocksCh
	}
	...
	ps.wrapped.AddSub(valuesCh, key.KeyString())
	//ps.wrapped.AddSubOnceEach(valuesCh, toStrings(keys)...)
	go func() {
		defer func() {
			close(blocksCh)
		...
		for {
			...
			case val, ok := <-valuesCh:
				...
				_, ok = val.(blocks.Block)
				if !ok {
					return
				}
				cblock, ok := val.(*blocks.CodedBlock)
				if !ok {
					fmt.Println("Debug: subc - receiving normal block")
					continue
				}
