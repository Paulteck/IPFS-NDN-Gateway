...
type update struct {
	...
	// DONT_HAVE message
	dontHaves []cid.Cid
	// number and cid of received coded blocks
	ksc map[cid.Cid]int
}
...
type change struct {
	...
	// new coded wants
	addc [] peermanager.CodedWant

	// cancel coded want
	cancelc []peermanager.CodedWant
}
...
func (sws *sessionWantSender) AddC(key cid.Cid, coding string, count int) {
	//fmt.Println("Debug: sws-addC", count)
	if count == 0 {
		return
	}
	cw:=peermanager.CodedWant{Cid: key, Coding: coding, Count: count}
	sl:= make([]peermanager.CodedWant,1)
	sl[0]=cw
	sws.addChange(change{addc: sl})
}

// Cancel is called when a request is cancelled
func (sws *sessionWantSender) Cancel(ks []cid.Cid) {
	//fmt.Println("Debug: sws-cancel", ks)
	if len(ks) == 0 {
		return
	}
...
// Cancel is called when a request is cancelled
func (sws *sessionWantSender) CancelC(key cid.Cid, coding string, count int) {
	//fmt.Println("Debug: sws-cancelc", key, count)
	cw:=peermanager.CodedWant{Cid: key, Coding: coding, Count: count}
	sl:= make([]peermanager.CodedWant,1)
	sl[0]=cw
	sws.addChange(change{cancelc: sl})
}


// Update is called when the session receives a message with incoming blocks
// or HAVE / DONT_HAVE
func (sws *sessionWantSender) Update(from peer.ID, ks []cid.Cid, haves []cid.Cid, dontHaves []cid.Cid, ksc map[cid.Cid]int) {
	hasUpdate := len(ks) > 0 || len(haves) > 0 || len(dontHaves) > 0 || len(ksc) > 0
	if !hasUpdate {
		return
	}

	sws.addChange(change{
		update: update{from, ks, haves, dontHaves, ksc},
	})
}
...
func (sws *sessionWantSender) onChange(changes []change) {
	...
	for _, chng := range changes {
		...
		for _, c := range chng.addc {
	//		fmt.Println("Debug: sws-onchanges-addc")
			sws.trackWantC(c)
		}
		...
		for _, c := range chng.cancelc {
			sws.untrackWantC(c)
			continue
			// MARS todo untrack wants
//			sws.untrackWant(c)
//			cancels = append(cancels, c)
		}
		...
		if chng.update.from != "" {
			fmt.Println("Debug: sws-update new peer", chng.update.from, len(chng.update.ks), len(chng.update.haves))
			// If the update includes blocks or haves, treat it as signaling that
			// the peer is available
			if len(chng.update.ks) > 0 || len(chng.update.haves) > 0 || len(chng.update.ksc) > 0 {
				p := chng.update.from
				availability[p] = true
	...
	fmt.Println("Debug: sws-oc Peers av", sws.spm.HasPeers())
...
func (sws *sessionWantSender) trackWant(c cid.Cid) {
	//fmt.Println("Debug: sws-trackwant", c)
...
func (sws *sessionWantSender) trackWantC(c peermanager.CodedWant) {
	//fmt.Println("Debug: sws-trackwantc")
	if _, ok := sws.wants[c.Cid]; ok {
		fmt.Println("Debug: sws-trackwantc found existing want")
		if sws.wants[c.Cid].coding == "" || sws.wants[c.Cid].coding == c.Coding {
			sws.wants[c.Cid].coding	= c.Coding
			// MARS todo: lock
			sws.wants[c.Cid].count += c.Count
		}
		return
	}

//	fmt.Println("Debug: sws-trackwantc-create")
	// Create the want info
	wi := newWantInfo(sws.peerRspTrkr)
	wi.coding=c.Coding
	wi.count=c.Count
	...
	// whether the peer has the block
	for _, p := range sws.spm.Peers() {
		sws.updateWantBlockPresence(c.Cid, p)
	}
}
...
func (sws *sessionWantSender) untrackWant(c cid.Cid) {
	fmt.Println("Debug: untrack want",c)
	if w, ok:=sws.wants[c]; ok{
		if w.count > 0 {return}
	}
	delete(sws.wants, c)
}

func (sws *sessionWantSender) untrackWantC(c peermanager.CodedWant) {
	wi, ok := sws.wants[c.Cid]
	if !ok {return}
	fmt.Println("Debug: sws-untrackwantc, cid", c.Cid ,"count", wi.count,"cancel", c.Count)
    if wi.count <= c.Count{
		delete(sws.wants, c.Cid)
		ks := []cid.Cid{c.Cid}
		sws.canceller.CancelSessionWants(sws.sessionID, ks)
		for p,_ := range wi.sentToC{
			sws.pm.SendCancelC(sws.ctx, p, c.Cid, wi.total)
		}
	} else {
		wi.count = wi.count - c.Count
		sws.wants[c.Cid]=wi
	}
}
...
func (sws *sessionWantSender) processUpdates(updates []update) []cid.Cid {
	...
	for _, upd := range updates {
		for _, c := range upd.haves {
			...
			wi, ok :=sws.wants[c]
			// If we haven't already received a block for the want or key is coded want
			if (ok && wi.count > 0) || !blkCids.Has(c) {
				// Update the block presence for the peer
				sws.updateWantBlockPresence(c, upd.from)
			}
...
type wantSets struct {
	wantBlocks *cid.Set
	wantHaves  *cid.Set
	wantCodeds *map[peermanager.CodedWant]struct{}
...
func (aw allWants) forPeer(p peer.ID) *wantSets {
	if _, ok := aw[p]; !ok {
		wantCodeds:=make(map[peermanager.CodedWant]struct{})
		aw[p] = &wantSets{
			wantBlocks: cid.NewSet(),
			wantHaves:  cid.NewSet(),
			wantCodeds: &wantCodeds,
		}
	}
	return aw[p]
}
...
//	fmt.Println("Debug: sendNextWants")
	...
	for c, wi := range sws.wants {
		...
		//fmt.Println("Debug: sws-snw, coding", wi.coding, "sendTo", wi.sentTo)
		// if wi.sentto != "" and coding==""?

		// We already sent a want-block to a peer and haven't yet received a
		// response yet
		if wi.sentTo != "" && !(wi.coding == "nc" && wi.count>0) {
			continue
		}

//		fmt.Println("Debug: sws-sendnextwants-coding", wi.coding)
		if wi.coding=="nc"{
			fmt.Println("Debug: sws-snw trying to send coded want", len(wi.sentToC), len(sws.spm.Peers()))
//			fmt.Println("Debug: found coded entry")
			if len(wi.sentToC) == 0{
				pc:=len(sws.spm.Peers())
				sendtoc:=wi.count/pc+1
				sendtoc=wi.count

 				fmt.Println("Debug: sending coded wants for", sendtoc, "packets to", len(sws.spm.Peers()))
				fmt.Println(sws.spm.Peers())

				for _,pe := range sws.spm.Peers(){
					(*toSend.forPeer(pe).wantCodeds)[peermanager.CodedWant{Cid: c, Coding: wi.coding, Count: sendtoc}]=struct{}{}
					wi.sentToC[pe]=sendtoc
	//					fmt.Println("Debug: to ", pe)
				}
			} else if len(newlyAvailable) > 0 {
				pc:=len(sws.spm.Peers())
				sendtoc:=wi.count/pc+1
				sendtoc=wi.count

				fmt.Println("Debug: sending new coded wants for", sendtoc, "packets to", len(newlyAvailable))
				fmt.Println(newlyAvailable)

				for _,pe := range newlyAvailable{
					(*toSend.forPeer(pe).wantCodeds)[peermanager.CodedWant{Cid: c, Coding: wi.coding, Count: sendtoc}]=struct{}{}
					wi.sentToC[pe]=sendtoc
				}
			}
		...
		// MARS todo sent list für gecodete (inkl. Anzahl?)
		// wenn liste leer, dann an alle n/p Anfrage
		// new available ?

		//MARS todo dont have wenn rang kleiner Anfrage?
		...
		// MARS debug
		//log.Errorw("session want sender <- want-block", "to", wi.bestPeer, "cid", c)
...
func (sws *sessionWantSender) sendWants(sends allWants) {
	...
	for p, snd := range sends {
		...
		ckeys := make([]peermanager.CodedWant, len(*snd.wantCodeds))
		i := 0
		for k := range *snd.wantCodeds {
    		ckeys[i] = k
    		i++
		}
//		fmt.Println("Debug: sws-sendwants-codedwants", i)
		sws.pm.SendWants(sws.ctx, p, wblks, whaves, ckeys)
...
func (sws *sessionWantSender) removeWant(c cid.Cid) *wantInfo {
	fmt.Println("Debug: sws-removeWant deleting", c)
	if wi, ok := sws.wants[c]; ok {
		if wi.count > 0 {
			return nil
		}
...
// Record which peer the want was sent to
func (sws *sessionWantSender) setWantSentToC(c cid.Cid, p peer.ID, count int) {
	if wi, ok := sws.wants[c]; ok {
		wi.sentToC[p] = count
	}
}
...
type wantInfo struct {
	...
	// MARS todo track received packets?
	// MARS stuff
	coding string
	count int
	total int
	sentToC map[peer.ID]int
}
