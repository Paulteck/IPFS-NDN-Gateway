...
type PeerManager interface {
	...
	SendWants(ctx context.Context, peerId peer.ID, wantBlocks []cid.Cid, wantHaves []cid.Cid, wantCodeds []bspm.CodedWant)
	...
	SendCancelC(context.Context, peer.ID, cid.Cid, int)
}
...
func (s *Session) ReceiveFrom(from peer.ID, ks []cid.Cid, haves []cid.Cid, dontHaves []cid.Cid, ksc map[cid.Cid]int) {
	tksc := make([]cid.Cid, 0, len(ksc))
	for k := range ksc {
		tksc = append(tksc, k)
	}
	...
	interestedRes := s.sim.FilterSessionInterested(s.id, ks, haves, dontHaves, tksc)
	ks = interestedRes[0]
	haves = interestedRes[1]
	dontHaves = interestedRes[2]
	tksc = interestedRes[3]
	kscr := make(map[cid.Cid]int, len(tksc))
	for _, k := range tksc {
		kscr[k] = ksc[k]
	}
	s.logReceiveFrom(from, ks, haves, dontHaves)

	fmt.Println("Debug: sess-receive from, blks", ks,", haves", haves)

	// Inform the session want sender that a message has been received
	s.sws.Update(from, ks, haves, dontHaves, kscr)

	if len(ks) == 0 {
		return
	}

	// MARS todo handleReceive for coded blocks
...
func (s *Session) GetBlock(parent context.Context, k cid.Cid) (blocks.Block, error) {
	//fmt.Println("Debug: session-getblock")
...
func (s *Session) GetBlocks(ctx context.Context, keys []cid.Cid) (<-chan blocks.Block, error) {
	//fmt.Println("Debug: session-getblocks")
...
func (s *Session) GetBlocksC(ctx context.Context, parent cid.Cid, coding string, count int) (<-chan blocks.Block, error) {
	//fmt.Println("Debug: session-getblocksC")
	ctx = logging.ContextWithLoggable(ctx, s.uuid)

	return bsgetter.AsyncGetBlocksC(ctx, s.ctx, parent, coding, count, s.notif,
		func(ctx context.Context, parent cid.Cid, coding string, count int) {
			select {
			case s.incoming <- op{op: opWantC, par: parent, coding: coding, count: count}:
			case <-ctx.Done():
			case <-s.ctx.Done():
			}
		},
		func(parent cid.Cid, coding string, count int) {
			select {
			case s.incoming <- op{op: opCancelC, par: parent, coding: coding, count: count}:
			case <-s.ctx.Done():
			}
		},
	)
...
func (s *Session) run(ctx context.Context) {
	...
	for {
		...
		case oper := <-s.incoming:
			...
			case opReceiveC:
				// Received blocks
				s.handleReceiveC(oper.keys)
			case opWant:
				// Client wants blocks
				s.wantBlocks(ctx, oper.keys)
			case opWantC:
				//fmt.Println("Debug: opwantc", oper.count)
				// Client wants blocks
				s.wantBlocksC(ctx, oper.par, oper.coding, oper.count)
			case opCancel:
			...
			case opCancelC:
				// s.sw.CancelPending(oper.keys)
				s.sws.CancelC(oper.par,oper.coding,oper.count)
			case opWantsSent:
			...
			default:
//				fmt.Println("Debug: panic", oper.op)
...
// MARS todo use sessionwants
// sim handling in sws
func (s *Session) handleReceiveC(ks []cid.Cid) {
	// Record which blocks have been received and figure out the total latency
	// for fetching the blocks
	wanted, totalLatency := s.sw.BlocksReceived(ks)
	if len(wanted) == 0 {
...
// MARS todo tracker
func (s *Session) wantBlocksC(ctx context.Context, key cid.Cid, coding string, count int) {
	fmt.Println("Debug: ss-wantBlocksC", count)
	if count > 0 {
		// Inform the SessionInterestManager that this session is interested in the keys
		s.sim.RecordSessionInterest(s.id, []cid.Cid{key})
		s.sim.RecordSessionInterestC(s.id, key, count)
		// Tell the sessionWants tracker that that the wants have been requested
		//s.sw.BlocksRequested(newks)
		// Tell the sessionWantSender that the blocks have been requested
		s.sws.AddC(key, coding, count)
	}
	...
	fmt.Println("Debug: wantBlocksC - no peers discovered ", key)
