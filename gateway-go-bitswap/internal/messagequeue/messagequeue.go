...
// Add want to the pending list
func (r *recallWantlist) AddC(c cid.Cid, priority int32, wtype pb.Message_Wantlist_WantType, coding string, count int) {
//	fmt.Println("Debug: mw-addc", count)
	r.pending.AddC(c, priority, wtype, coding, count)
}

// Remove wants from both the pending list and the list of sent wants
func (r *recallWantlist) Remove(c cid.Cid) {
//	fmt.Println("Debug: mq-remove", c)
	r.pending.Remove(c)
	r.sent.Remove(c)
	delete(r.sentAt, c)
}
...
//	fmt.Println("Debug: mq-removetype", c, wtype)
...
func (r *recallWantlist) MarkSent(e bswl.Entry) bool {
	...
	// MARS todo entries der sent list werden wieder angefragt, wenn nicht remove aufgerufen wird
	// MARS todo want management
	if e.Coding != "" {
		return true
	}
...
func newMessageQueue(
	...
	return &MessageQueue{
		...
		cancels:             cid.NewSet(),
		codedCancels:        newRecallWantList(),
		codedWants: 		 cid.NewSet(),
...
func (mq *MessageQueue) AddBroadcastWantHaves(wantHaves []cid.Cid) {
	...
	fmt.Println("Debug: adding broadcast wants", wantHaves)
...
func (mq *MessageQueue) AddWants(wantBlocks []cid.Cid, wantHaves []cid.Cid, wantCodeds []peermanager.CodedWant) {

	if len(wantBlocks) == 0 && len(wantHaves) == 0 && len(wantCodeds) == 0 {
		return
	}
	...
	// MARS todo Nachbearbeitung?
	for _, c := range wantCodeds {
//		fmt.Println("Debug: mq-addwants cid to codedwants", c)
		mq.codedWants.Add(c.Cid)
		mq.peerWants.AddC(c.Cid, mq.priority, pb.Message_Wantlist_Block, c.Coding, c.Count)
		mq.priority--

		// We're adding a want-block for the cid, so clear any pending cancel
		// for the cid
		mq.cancels.Remove(c.Cid)
	}
...
//		fmt.Println("Debug: mq-addcancel", c)
		if mq.codedWants.Has(c) {
//			fmt.Println("coded want, continue")
			continue
		}
...
func (mq *MessageQueue) AddCancelC(key cid.Cid, count int) {
	// MARS todo add coded cancel
	mq.codedCancels.AddC(key, mq.priority, pb.Message_Wantlist_Block, "nc", count)
	mq.signalWorkReady()
}
...
func (mq *MessageQueue) extractOutgoingMessage(supportsHave bool) (bsmsg.BitSwapMessage, func()) {
	...
	fmt.Println("Debug: pending peerentries", peerEntries)
	bcstEntries := mq.bcstWants.pending.Entries()
	fmt.Println("Debug: pending broadcastentries", bcstEntries)
	cancels := mq.cancels.Keys()
	codedCancels := mq.codedCancels.pending.Entries()
	if !supportsHave {
	...
	for _, c := range codedCancels {
		fmt.Println("Debug: mq-extractmessage adding coded cancel", )
		msgSize += mq.msg.CancelCoded(c.Cid, c.Coding, c.Count)
		mq.codedCancels.Remove(c.Cid)
	...
	for _, e := range peerEntries {
		if e.Count > 0 {
			mq.codedWants.Remove(e.Cid)
		}
		msgSize += mq.msg.AddCodedEntry(e.Cid, e.Priority, e.WantType, true, e.Coding, e.Count)
		sentPeerEntries++
