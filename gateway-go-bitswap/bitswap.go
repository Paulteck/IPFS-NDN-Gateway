...
func (bs *Bitswap) GetBlocks(ctx context.Context, keys []cid.Cid) (<-chan blocks.Block, error) {
	//fmt.Println("Debug: bitswap-getblocks")
...
func (bs *Bitswap) receiveBlocksFrom(ctx context.Context, from peer.ID, blks []blocks.Block, haves []cid.Cid, dontHaves []cid.Cid) error {
	/*for _,blk:=range blks{
		fmt.Printf("Debug: bs-receiveBlocksFrom from=%s, cid=%s", from, blk.Cid() )
		fmt.Println("")
	}*/
	select {
	...
	// split coded uncoded
	wanted:=make([]blocks.Block,0)
	wantedc:=make([]*blocks.CodedBlock,0)
	for _, b := range blks{
		if c, ok:=b.(*blocks.CodedBlock); ok{
			wantedc=append(wantedc, c)
		} else {
	...
	if from != "" {
		...
		//fmt.Println("Debug: Peers ID: ", from)
		wanted, notWanted, _ = bs.sim.SplitWantedUnwanted(blks)
		for _, b := range notWanted {
			log.Debugf("[recv] block not in wantlist; cid=%s, peer=%s", b.Cid(), from)
			//fmt.Println("Debug: NotWanted Block CID: ", b.Cid())
		}

		for _, b := range wanted {
			if len(b.RawData()) > 10000 {
				fmt.Printf("Debug: [recv] block in wantlist ipfs; cid=%s, peer=%s\n", b.Cid(), from)
				}
			log.Debugf("[recv] block in wantlist; cid=%s, peer=%s", b.Cid(), from)
			}
		for _, b := range wantedc {
			log.Debugf("[recv] coded block in wantlist; cid=%s, peer=%s", b.Parent(), from)
			if !bs.sim.HasDecoder(b.Parent()){
				root, err := bs.blockstore.Get(b.Parent())
				if err != nil {
					fmt.Println(err)
					continue
				}
				rootn, err := ipld.Decode(root)
				symbols:=len(rootn.Links())
				blockSize := rootn.Links()[0].Size-14

				bs.sim.
					CreateDecoder(b.Parent(), symbols, int(blockSize))
			}
		}
		wantedc, _ = bs.sim.SplitWantedUnwantedC(wantedc)
	}

	// Put wanted blocks into blockstore
	if len(wanted) > 0 || len(wantedc) > 0 {
		// MARS DEBUG die nächsten drei Zeilen
		for _, b := range wantedc {
			fmt.Printf("Debug: [recv] block in wantlist mars; cid=%s, peer=%s\n", b.Parent(), from)
		}
		...
		err = bs.putCoded(wantedc)
		if err != nil {
			log.Errorf("Error writing %d coded blocks to datastore: %s", len(wantedc), err)
			return err
		}
	}
	...
	if from != "" {
		fmt.Println("Debug: bitswap-receive blks", allKs, "have", haves)
	...
	Ksc := make(map[cid.Cid] int)
	for _,b := range wantedc{
		if c, ok := Ksc[b.Parent()]; ok{
			Ksc[b.Parent()] = c+1
		} else {
			Ksc[b.Parent()] = 1
		}
	...
	bs.sm.ReceiveFrom(ctx, from, allKs, haves, dontHaves, Ksc)

	fmt.Println("Debug: bitswap - reproviding coded blocks", len(wantedc))
	// Send wanted blocks to decision engine
	bs.engine.ReceiveFrom(from, wanted, haves, wantedc)
	...
	for _, b := range wantedc {
		fmt.Println("Debug: publishing coded block", b.Parent())
		bs.notif.Publish(b)
	}
...
func (bs *Bitswap) putCoded(blks []*blocks.CodedBlock) error {
	b := make([]blocks.Block, len(blks))
	for i := range blks {
	//	fmt.Println("Debug: put coded blocks len", len(blks[i].RawData()))
		b[i] = blks[i]
	}
	err := bs.blockstore.PutMany(b)
	if err != nil {
		return err
	}
	bs.codingLk.Lock()
	defer bs.codingLk.Unlock()

	files:=make(map[cid.Cid] *os.File)

	for _,b := range blks {
		f, ok := files[b.Parent()]
		if  !ok {
			f, err= os.OpenFile(os.Getenv("IPFS_PATH") + "/nc/" + b.Parent().String(), os.O_APPEND|os.O_WRONLY, 0644)
			if err!=nil{
				_ = os.Mkdir(os.Getenv("IPFS_PATH")+"/nc", 0755)
				os.Create(os.Getenv("IPFS_PATH") + "/nc/" + b.Parent().String())
				f, err= os.OpenFile(os.Getenv("IPFS_PATH") + "/nc/" + b.Parent().String(), os.O_APPEND|os.O_WRONLY, 0644)
				if err != nil {
					return err
				}
		...
		fstat, err:= os.Stat(os.Getenv("IPFS_PATH")+"/nc/"+b.Parent().String())
		has := fstat.Size()/47
		fmt.Println("Debug: bitswap size before add ", has)
		_, err = fmt.Fprintln(f, b.Cid().String())
		if err != nil {
			fmt.Println("Debug: error writing cid to file", err)
			return err
		}
	}
	return nil
