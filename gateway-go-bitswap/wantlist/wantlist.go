...
func NewRefEntryC(c cid.Cid, p int32, cd string, cn int) Entry {
	return Entry{
		Cid:      c,
		Priority: p,
		Coding:   cd,
...
// Add adds an entry in a wantlist from CID & Priority, if not already present.
func (w *Wantlist) AddC(c cid.Cid, priority int32, wantType pb.Message_Wantlist_WantType, coding string, count int) bool {
	e, ok := w.set[c]
...
// Remove removes count for the given cid from the wantlist.
func (w *Wantlist) RemoveC(c cid.Cid, co int) bool {
	wi, ok := w.set[c]
	if !ok {
		return false
	}
