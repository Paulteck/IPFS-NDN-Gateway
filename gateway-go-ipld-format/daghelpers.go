...
func GetNodes(ctx context.Context, ds NodeGetter, keys []cid.Cid) []*NodePromise {
    //fmt.Println("Debug: daghelper-getnodes")
...
// GetNodes returns an array of 'FutureNode' promises, with each corresponding
// to the key with the same index as the passed in keys
func GetNodesC(ctx context.Context, ds NodeGetter, parent cid.Cid, coding string, ccount int) []*NodePromise {
	//fmt.Println("Debug: GetNodesC")
	// Early out if no work to do
	if ccount == 0 {
		return nil
	}

	promises := make([]*NodePromise, ccount)
	for i := 0; i < ccount; i++ {
		promises[i] = NewNodePromise(ctx)
	}
	...
	go func() {
		...
		nodechan := d.GetManyC(ctx, parent, coding, ccount)
                fmt.Println("Debug: Daghelpers - GetNodesC - nodechan finish")
		for count := 0; count < ccount; {
			select {
			case opt, ok := <-nodechan:
				if !ok {
					fmt.Println("Debug: getmanyc not ok")
					for _, p := range promises {
						p.Fail(ErrNotFound)
					}
					return
				...
				if opt.Err != nil {
					fmt.Println("Debug: getmanyc nodeoption err", opt.Err)
					for _, p := range promises {
						p.Fail(opt.Err)
					}
					return
				...
				nd := opt.Node
                                fmt.Println("Debug: Daghelpers - getmany - sending  count: ", count)
				promises[count].Send(nd)
				count++
			case <-ctx.Done():
				return
			}
