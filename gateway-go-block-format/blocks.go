...
// NewCodedBlockWithCid creates a new block from coded packets
func NewCodedBlockWithCid(data []byte, c cid.Cid, p cid.Cid) (*CodedBlock, error) {
	if u.Debug {
		chkc, err := c.Prefix().Sum(data)
		if err != nil {
			return nil, err
	...
	b := BasicBlock{data: data, cid: c}
	return &CodedBlock{b, p}, nil
}
...
// Cid returns the content identifier of the block.
func (c *CodedBlock) Cid() cid.Cid {
	return c.cid
}

// Parent returns the content identifier of the header block.
func (c *CodedBlock) Parent() cid.Cid {
	return c.parent
}

func (c *CodedBlock) String() string {
	return fmt.Sprintf("[Block %s, Par %s]", c.Cid(), c.Parent())
}
