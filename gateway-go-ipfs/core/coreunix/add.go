...
func (adder *Adder) outputDirs(path string, fsn mfs.FSNode) error {
	...
	case *mfs.File:
		// MARS
		if strings.HasPrefix(adder.Chunker, "nc") {
			...
			f, err := os.Create(os.Getenv("IPFS_PATH") + "/nc/" + stat.Hash)
			if err != nil {
				return err
			}
			defer f.Close()
			...
			for _, link := range node.Links() {
				//fmt.Println(link.Cid)
				fmt.Fprintln(w, link.Cid)
			}
			return w.Flush()
		}
		return nil
...
func (adder *Adder) AddAllAndPin(file files.Node) (ipld.Node, error) {
	//s,err:= file.Size()
	//fmt.Println("Debug: addallandpin", s, err)
	if adder.Pin {
...
func (adder *Adder) addFileNode(path string, file files.Node, toplevel bool) error {
	//s,e:=file.Size()
	//fmt.Println("Debug: addFileNode", path, s,e )
	defer file.Close()
