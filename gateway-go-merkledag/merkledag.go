...
// Add adds a node to the dagService, storing the block in the BlockService
func (n *dagService) AddCoded(ctx context.Context, blk blocks.Block) error {
	if n == nil { // FIXME remove this assertion. protect with constructor invariant
		return fmt.Errorf("dagService is nil")
	}

	return n.Blocks.AddBlock(blk)
}
...
func (sg *sesGetter) GetMany(ctx context.Context, keys []cid.Cid) <-chan *ipld.NodeOption {
	//fmt.Println("Debug: sg.getmany")
	return getNodesFromBG(ctx, sg.bs, keys)
}


// GetMany gets many nodes at once, batching the request if possible.
func (sg *sesGetter) GetManyC(ctx context.Context, parent cid.Cid, coding string, count int) <-chan *ipld.NodeOption {
	//fmt.Println("Debug: sg.getmany")
	return getNodesFromBGC(ctx, sg.bs, parent, coding, count)
}
...
func FetchCodedCh(ctx context.Context, root cid.Cid, coding string, count int, serv ipld.DAGService ) <-chan *ipld.NodeOption  {
	var ng ipld.NodeGetter = NewSession(ctx, serv)

	// get root node for session peers
	// pin or not pin?
	// nope, wird schon in resolvenode geholt/ ist eh doof
	/*_, err := ng.Get(ctx, root)
	if err != nil {
	...
	ds, ok:= ng.(ipld.NodeGetterC)
	if !ok {
		fmt.Errorf("FetchCoded: error dagservice assertion")
	}
	return ds.GetManyC(ctx, root, coding, count)
}
...
func getNodesFromBG(ctx context.Context, bs bserv.BlockGetter, keys []cid.Cid) <-chan *ipld.NodeOption {
	//fmt.Println("Debug: getnodesfrombg")
	keys = dedupKeys(keys)
...
func getNodesFromBGC(ctx context.Context, bs bserv.BlockGetter, parent cid.Cid, coding string, ccount int) <-chan *ipld.NodeOption {
	//fmt.Println("Debug: getnodesfrombgC")

	out := make(chan *ipld.NodeOption, ccount)

	//fmt.Println(reflect.TypeOf(bs))
	b, ok := bs.(bserv.BlockGetterC)
	if !ok {
		fmt.Println("Error: BG assertion failed")
	}
	blocks := b.GetBlocksC(ctx, parent, coding, ccount)
	var count int
	...
	go func() {
		...
		for {
			select {
			case b, ok := <-blocks:
                                fmt.Println("Debug: getnodesfrombgc - incoming block")
				if !ok {
					if count != ccount {
						out <- &ipld.NodeOption{Err: fmt.Errorf("failed to fetch all nodes")}
					}
				...
				if err != nil {
					...
					return
                                ...
                                fmt.Println("Debug: getnodesfrombgc - outgoing block")
				out <- &ipld.NodeOption{Node: nd}
				count++
